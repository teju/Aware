...
class TempGraphFragment : BaseFragment() ,View.OnClickListener, OnChartValueSelectedListener {
    companion object {
        val DAILY = 1
        val WEEKLY = 2
        val MONTHLY = 3
    ...
    override fun onCreateView(
        ...
        v = inflater.inflate(R.layout.fragment_temp_graph, container, false)
        return v
    }
    ...
    fun setXaxisData(which : Int) {
        ...
        values.clear()
        if(which == DAILY) {
            values = DailyData().getXAxisDailyTemp(activity!!,"")
        } else if(which == WEEKLY) {
            values = WeeklyData().getXAxisTempWeekly(activity!!,"")
        } else if(which == MONTHLY) {
            values = MonthlyData().getXAxisTempMonthly(activity!!,"")
        }
        renderData(which)
