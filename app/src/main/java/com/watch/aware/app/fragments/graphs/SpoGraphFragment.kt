...
import kotlinx.android.synthetic.main.fragment_spo_graph.*
import java.util.*
import kotlin.collections.ArrayList


class SpoGraphFragment : BaseFragment() ,View.OnClickListener, OnChartValueSelectedListener {
    companion object {
        val DAILY = 1
        val WEEKLY = 2
        val MONTHLY = 3
    ...
    override fun onCreateView(
        ...
        v = inflater.inflate(R.layout.fragment_spo_graph, container, false)
        return v
    }
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val stepsArray = db.getAllSpoRate("Where SpoRate != 0  ORDER by Id DESC")
        if(stepsArray!= null && stepsArray?.size != 0) {
            stepsCount.text = stepsArray.get(0).spoRate.toString()
        }
        setXaxisData(DAILY)
    }
    ...
    fun setXaxisData(which : Int) {
        ...
        values.clear()
        if(which == DAILY) {
            values = DailyData().getXAxisDailySpo(activity!!,"")
        } else if(which == WEEKLY) {
            values = WeeklyData().getXAxisSpoeWeekly(activity!!,"")
        } else if(which == MONTHLY) {
            values = MonthlyData().getXAxisSpoMonthly(activity!!,"")
        }
        renderData(which)
