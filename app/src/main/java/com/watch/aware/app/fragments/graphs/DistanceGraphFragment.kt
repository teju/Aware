...
class DistanceGraphFragment : BaseFragment() ,View.OnClickListener, OnChartValueSelectedListener {
    companion object {
        val DAILY = 1
        val WEEKLY = 2
        val MONTHLY = 3
    ...
    override fun onCreateView(
        ...
        v = inflater.inflate(R.layout.fragment_distance_graph, container, false)
        return v
    }
    ...
    fun setXaxisData(which : Int) {
        ...
        if(which == DAILY) {
            ...
            values = WeeklyData()
                .getXAxisStepWeekly(activity!!,"dist")
        } else if(which == MONTHLY) {
            values = MonthlyData().getXAxisStepMonthly(activity!!,"dist")
        }
        renderData(which)
