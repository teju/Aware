...
import kotlinx.android.synthetic.main.fragment_heartrate_graph.*
...
class HeartRateGraphFragment : BaseFragment() ,View.OnClickListener, OnChartValueSelectedListener {
    companion object {
        val DAILY = 1
        val WEEKLY = 2
        val MONTHLY = 3
    ...
    override fun onCreateView(
        ...
        v = inflater.inflate(R.layout.fragment_heartrate_graph, container, false)
        return v
    }
    ...
    fun setStepsData(which: Int) {
        try {
            if (which == StepsGraphFragment.DAILY) {
                ...
                val dteps = WeeklyData().getWeeklyHeartData(day - 1, "", activity!!)
                ...
                val heartrate = MonthlyData().getMonthlyHeartData(day, "", activity!!)
    ...
    fun setXaxisData(which : Int) {
        ...
        values.clear()
        if(which == DAILY) {
            values = DailyData().getXAxisDailyHeart(activity!!,"")
        } else if(which == WEEKLY) {
            values = WeeklyData().getXAxisHeartRateWeekly(activity!!,"")
        } else if(which == MONTHLY) {
            values = MonthlyData().getXAxisHeartonthly(activity!!,"")
        }
        renderData(which)
